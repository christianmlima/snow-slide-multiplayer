...
class SnowSlideTPSMasterEngine {
  ...
  private garagePos = new THREE.Vector3(-44, 0, -28);
  private garageCounterPos = new THREE.Vector3(-44, 0, -29);

  private hatShopPos = new THREE.Vector3(-44, 0, 38);
  private hatShopCounterPos = new THREE.Vector3(-44, 0, 39);

  private atelierPos = new THREE.Vector3(12, 0, 44);
  private atelierCounterPos = new THREE.Vector3(12, 0, 45);

  private tavernPos = new THREE.Vector3(46, 0, 16);
  ...
  private snowballBurstPoints!: THREE.Points;
  private burstParticles: { x: number; y: number; z: number; vx: number; vy: number; vz: number; life: number; maxLife: number; size: number }[] = [];
  ...
  private emitSnowSpray(count: number, lateralBoost = 0, originX?: number, originY?: number, originZ?: number) {
  ...
  // =========================================================================
  private createSnowballBurstSystem() {
    const count = 300;
    const geo = new THREE.BufferGeometry();
    const positions = new Float32Array(count * 3);
    for (let i = 0; i < count * 3; i++) positions[i] = 0;
  ...
  private emitSnowballDisintegration(ox: number, oy: number, oz: number, isCharacterHit = false) {
  ...
  private createRoundedWallGeometry(w: number, h: number, d: number, radius = 0.32): THREE.BufferGeometry {
  ...
  // =========================================================================
  private createWoodenParkBench(x: number, z: number, rotY: number): THREE.Group {
  ...
  private createCarnivalBooth(x: number, z: number, rotY: number = 0): THREE.Group {
    const booth = new THREE.Group();
  ...
  private createSnowballWarPortal(x: number, z: number, rotY: number = 0): THREE.Group {
  ...
  private loadHubScene() {
    ...
    this.scene.add(this.createTextSignboard(
      ...
      '🏔️ EMBARQUE PARA A CORRIDA DE DESCIDA',
      '#38bdf8'
    ));
    ...
    this.scene.add(this.createTextSignboard(
      31, 16, -Math.PI / 2,
      '🏆 TAVERNA DOS CAMPEÕES',
      '🌟 SALÃO DE RECORDES & HALL DA FAMA',
      '#eab308'
    ));
  ...
  private setupUIAndControls() {
    ...
    if (snowballBtn) {
      const handleSnowballPress = (e: Event) => {
        e.stopPropagation();
        if (this.isAnyModalOpen()) return;
        this.initAudio();
      ...
      const handleSnowballRelease = (e: Event) => {
        e.stopPropagation();
    ...
    if (aimBtn) {
      aimBtn.addEventListener('pointerdown', (e) => {
        ...
        if (this.currentScene === 'HUB' || this.currentScene === 'SNOWBALL_WAR') {
          ...
          const crosshair = document.getElementById('hub-crosshair');
          if (crosshair) crosshair.classList.toggle('aiming', this.isAimingDownSights);
        }
      });
    }
