...
class SnowSlideTPSMasterEngine {
    ...
    carnivalBoothPos = new THREE.Vector3(18, 0, 16);
    carnivalTargets = [];
    shootingScore = 0;
    shootingHits = 0;
    shootingShots = 0;
    ...
    snowballWarPortalPos = new THREE.Vector3(-32, 0, 16);
    warBots = [];
    warObstacles = [];
    warPlayerHealth = 3;
    warKOs = 0;
    ...
    throwSnowball(chargeRatio = 0.5) {
        ...
        if (this.isAnyModalOpen())
            return;
        const now = Date.now();
        if (now - this.lastSnowballTime < 200)
            return;
    ...
    createCarnivalBooth(x, z) {
        ...
        for (let i = 0; i < stripeCount; i++) {
            const isRed = i % 2 === 0;
            const stripe = new THREE.Mesh(new THREE.BoxGeometry(stripeWidth, 0.25, 6.4), isRed ? redCanvasMat : whiteCanvasMat);
        ...
        for (let i = 0; i < stripeCount; i++) {
            const fringe = new THREE.Mesh(new THREE.ConeGeometry(0.42, 0.6, 4), (i % 2 === 0) ? redCanvasMat : whiteCanvasMat);
    ...
    loadHubScene() {
        ...
        this.scene.add(this.createTextSignboard(18, 11.2, Math.PI, '🎯 TIRO AO ALVO FESTIVO', '🎪 PATINHOS, PRÊMIOS & DIVERSÃO', '#f43f5e'));
        this.hubColliders.push({
            type: 'box',
        ...
        this.scene.add(this.createTextSignboard(-27.5, 14, -Math.PI / 4, '❄️ ARENA GUERRA DE NEVE', '⚔️ FORTES, TRINCHEIRAS & LABIRINTO', '#06b6d4'));
        this.hubColliders.push({
            type: 'box',
    ...
    spawnWarBots() {
        ...
        for (let i = 0; i < botConfigs.length; i++) {
            ...
            this.warBots.push({
                ...
                type: cfg.type,
                group: model,
                torso: model.children[0],
                head: model.children[0].children[2],
                armL: model.children[0].children[3],
                armR: model.children[0].children[4],
                footL: model.children[1],
                footR: model.children[2],
                tail: null,
